...
import { DefaultCategoryIcon, CheckMarkCircleIcon, CheckMarkIcon, ClockIcon, LocationPinIcon, TagIcon, UpvoteIcon, WrenchIcon, TextIcon, CheckMarkDoneIcon, LeftArrowIcon } from '../../components/Icons';
...
type IssueDetailRouteProp = RouteProp<
  { IssueDetails: { issue?: Issue | GetNearbyIssueResponse, id?: string } },
  'IssueDetails'
>;
...
const IssueDetailScreen = () => {
  ...
  const [issue, setIssue] = useState<Issue | GetNearbyIssueResponse | null>();
  ...
  const [resolvedAddress, setResolvedAddress] = useState('No address available');
  ...
  useEffect(() => {
    const getIssue = async () => {
      ...
      if (route.params.issue) {
        ...
        const i = await issuesApi.getIssueById(route.params.id)
        setIssue(i)
      }
  ...
  useEffect(() => {
    if (issue) {
      setResolvedAddress(issue.address || "No address available")
      setUpvoteCount(issue.upvoteCount ?? 0)
      setPhotoURLS(issue.photos.map((photo) => photo.url))
      setCategory(issue.category.replace(/_/g, " ").toLowerCase())
    }
    ...
    return (
      <View style={{ ...styles.page, }}>
        ...
        <Header title={issue.title}
          onBackPress={() => route.params.id ? navigation.replace("Nearby Issues", {}) : navigation.goBack()}
          lineNum={lineNum}
  ...
  return (
    <View style={{ ...globalStyles.container, justifyContent: "center", alignItems: "center" }}>
      <Text style={globalStyles.heading1}>There's been an error</Text>
      <WrapperButton style={{ backgroundColor: palette.ckLightGray, paddingHorizontal: spacing.md }}
        onPress={() => route.params.id ? navigation.replace("Nearby Issues", {}) : navigation.goBack}>
        <LeftArrowIcon size={typography.sizeXxxl} />
      </WrapperButton>
    </View>
  )
